...
//BOMBA----------------------------------------------------------------------------------------------------------------
const int BOMBA = 16; //pino da bomba
int tempo = 0;
...
void irrigar() {
  contar_tempo();
  if (umidade_solo<60 && tempo>=10000) { //escolhe o tempo que ela pode ficar sem receber água
    digitalWrite(BOMBA,HIGH);
    delay(10000); //tempo que a bomba fica ligada
    tempo = 0;  
  }
...
void ler_umidade_solo() {
  valorLido = analogRead(A0);
  umidade_solo = ((-25*valorLido + 200)/254)+100;
}
...
void sub_pub() {
  ...
  if (now - lastMsg > 5000) {                                            
    lastMsg = now;                                                       
    ++value;                                                             
    snprintf (msg, 120, "{\"code\": \"%s\", \"tipo\": %s, \"temp\": %f, \"umidade_ar\": %f, \"umidade_solo\": %i}", serial_code_estufa,tipo,temperatura_json,umidade_ar,umidade_solo);//MENSAGEM_A_SER_ENVIADA              
